...
//		// 作业：
		...
		final StatefulRedisConnection<String, String> connect = SentinelLettuce.getConnect();
		connect.async().set("uptime3", new Long(System.currentTimeMillis()).toString());
//		connect.sync().set("uptime3", new Long(System.currentTimeMillis()).toString());
		final RedisFuture<String> uptime3 = connect.async().get("uptime3");
		try {
			System.out.println(uptime3.get());
		} catch (InterruptedException e) {
			e.printStackTrace();
		} catch (ExecutionException e) {
			e.printStackTrace();
		}
//		System.out.println(connect.sync().get("uptime3"));
		connect.close();
...
//		System.out.println(jedis1.info().toString());
		jedis1.set("ww","ww01");
		Jedis jedis2 = new Jedis("localhost", 6380);
		System.out.println(jedis2.get("ww"));
		jedis2.slaveofNoOne();
		jedis2.set("wwtest","tt");
		jedis1.slaveof("redis1",6379);

		jedis2.set("ww01","ww0101");
		System.out.println(jedis1.get("ww01"));
